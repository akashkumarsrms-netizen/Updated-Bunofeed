...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // GOOGLE SHEET WEB APP URL - Replace this with your actual Web App URL after deploying from Google Apps Script
  const GOOGLE_SHEET_WEB_APP_URL = 'https://script.google.com/macros/s/AKfycbzUSZ3kXfdmlYR-HuRDLi4q0hvpwSvBhBoKHqAg1AtVYJBLjR-q_SZc5djUZ8ZXmPTXNQ/exec';
  ...
  async function syncToGoogleSheets(orderData) {
    if (!GOOGLE_SHEET_WEB_APP_URL || GOOGLE_SHEET_WEB_APP_URL.includes('YOUR_GOOGLE_APPS_SCRIPT')) {
    ...
    try {
      await fetch(GOOGLE_SHEET_WEB_APP_URL, {
        method: 'POST',
        mode: 'no-cors', // standard for Apps Script web apps unless CORS is specifically handled
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  secret: "BUNOFEEDBaheri243201_2026",
  data: orderData
})
      });
    } catch (e) {
